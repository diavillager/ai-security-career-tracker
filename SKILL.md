...
5. `parse_trend_observations`로 구조를 검사한 뒤 Trends DB의 기존 Original URL을 조회하고 `plan_trend_update`로 게시일, 출처, 복수 Related Roles, 복수 Domain과 정규화된 URL 중복을 모두 검증합니다.
...
Trends DB의 Domain은 AI, Security, AI × Security 중 하나 이상을 담는 Multi-select여야 합니다. 기존 DB가 단일 Select이면 실제 쓰기 전에 schema 변경 대상을 사용자에게 보여주고 승인을 받으며, 대체 DB를 만들지 않습니다.
