...
- Skill 이름, 기능명, 브랜치명, 코드 식별자, 설정 키, 명령어와 내부 enum은 영어 원문을 유지합니다. Notion 속성명과 옵션은 한글을 사용하되 `AI`, `Security`, `AI × Security`, `GitHub`는 영어 원문을 유지합니다.
...
- Job Posting마다 원문의 정확한 `job_title`을 기록하고, 서로 다른 원문 직무명을 하나의 observation이나 Candidate로 합치지 않습니다.
- 새 직무는 내부 상태 `Candidate`를 Notion의 `상태` 옵션 `후보`로 저장합니다. 사용자가 승인하기 전에는 Trend Update에 사용하지 않습니다.
...
- 먼저 Roles DB를 만들고, 이후 `관련 직무`가 Roles data source를 가리키는 Trends DB를 만듭니다.
...
1. 실행 시작 시 `run_id`, 검색 기간, 기존 Roles DB의 모든 `직무명`과 `상태`를 한 번 확정합니다.
...
- 검색 전에 모든 기존 `직무명`과 `상태`를 불러와 세 Agent가 같은 snapshot을 사용하게 합니다. Candidate, Approved, Rejected는 모두 정규화된 직무명이 같은 새 Candidate 생성을 막습니다.
...
- Job Posting의 `job_title`이 observation의 Role Name과 정규화 후 다르면 Python 구조 검사를 통과시키지 않습니다. 책임이 유사해도 서로 다른 직무명은 각자 근거 요건을 충족해야 합니다.
...
- Notion의 `근거 출처`에는 `format_candidate_evidence_sources` 결과를 사용해 복제본을 포함한 모든 URL과 근거 그룹을 남깁니다. `메모`에는 `format_candidate_evidence_note` 결과를 포함해 독립 근거 수, 보존 URL 수와 동일 공고 그룹을 구분합니다.
...
2. 검토 대상 `직무명`, 현재 `상태`와 page ID를 Roles DB에서 다시 조회합니다.
...
5. `build_notion_role_updates`로 변경할 `상태`, `최근 검토일`과 필요한 `메모`를 만듭니다.
6. 실제 Notion 쓰기 직전에 사용자에게 변경 목록을 보여주고 확인을 받습니다.
7. 확인된 Roles DB page에만 속성을 적용합니다. 승인 메모가 비어 있으면 기존 `메모`를 지우지 않습니다.
...
사용자가 승인된 직무의 최신 동향 수집을 요청하면 [references/trend-update.md](references/trend-update.md)를 읽고 다음 순서로 처리합니다.

1. 연결된 Notion workspace, 프로젝트 페이지와 저장된 Roles DB·Trends DB를 확인합니다.
2. Roles DB에서 모든 `직무명`, `직무 분야`, `상태`와 page ID를 한 번 조회합니다.
3. `build_trend_search_tasks`로 실행 시작 시점의 Approved 직무만 검색 작업으로 만듭니다. Candidate와 Rejected는 제외합니다.
4. Codex 웹 검색으로 기본 최근 7일의 기술·산업·연구·제품·보안 동향을 찾고 `references/classification-relations.md`에 따라 Source Type, 사실 중심의 Summary, 직무 관련 Key Insight, 하나 이상의 Domain과 관련 Approved 직무를 분류합니다. 제목 exact match만으로 Related Roles를 정하지 않습니다.
5. `parse_trend_observations`로 구조를 검사한 뒤 Trends DB의 기존 `원문 URL`을 조회하고 `plan_trend_update`로 게시일, 출처, 복수 `관련 직무`, 복수 `관련 분야`와 정규화된 URL 중복을 모두 검증합니다.
...
Trends DB의 `관련 분야`는 AI, Security, AI × Security 중 하나 이상을 담는 Multi-select여야 합니다. 기존 DB가 단일 Select이면 실제 쓰기 전에 schema 변경 대상을 사용자에게 보여주고 승인을 받으며, 대체 DB를 만들지 않습니다.
