...
이 기능은 Trend Update가 찾은 원문을 PRD의 `Source Type`, `Domain`, `Related Roles`, `Summary`, `Key Insight` 구조로 검증하고 Notion 속성에 연결합니다. 분류값을 추측으로 채우거나 Candidate와 Rejected 직무를 관계에 포함하지 않습니다.
...
`Domain`은 콘텐츠의 실질적 주제를 기준으로 AI, Security, AI × Security 중 하나 이상을 선택합니다. 같은 값을 반복하지 않습니다.
...
- `Summary`는 원문이 말하는 사실을 간결하게 정리합니다.
- `Key Insight`는 그 사실이 연결된 직무의 형성, 책임, 기술 요구나 시장 이해에 주는 의미를 설명합니다.
...
5. 모든 항목이 통과한 뒤 `build_notion_trend_pages`가 `Domain`을 `multi_select`, `Related Roles`를 `relation`으로 변환합니다.
